/** Project: Solo Lab2 Assignment Abstraction, Inheritance, Polymorphism: Monsters
 * Purpose Details: Create a Parent Class and Child Classes that inherit methods and characteristics from the parent
 * Course: IST 242 Section 001
 * Author: Conner Davis
 * Date Developed: 2/8/2026
 * Last Date Changed: 2/9/2026
 * Rev: 2/9/2026

 */

package monsters;

public class Main {
    public static void main(String[] args) {
        Monster z1 = new Zombie(Weapons.FISTS, SpecialPowers.SUPER_STRENGTH, 2,"Green", "Guy", 100, 12);
        Monster s1 = new Skeleton(Weapons.BOW, SpecialPowers.SUPER_SPEED,0, "White", "Gordon", 80, 206);
        Monster w1 = new Wither(Weapons.SWORD, SpecialPowers.SUPER_STRENGTH,2, "Gray", "Gary", 200, Boolean.TRUE);
        Monster s2 = new Spider(Weapons.FANGS, SpecialPowers.POISON,6, "Red", "Peter", 150, 8);
        Monster w2 = new Witch(Weapons.POTIONS, SpecialPowers.CLAIRVOYANCE,2, "Green", "Griselda", 110, 9);

        System.out.println("Mob 1: " + z1.getName());
        System.out.println(z1.getColor());
        z1.move();
        z1.superpower();

        System.out.println("Mob 2: " + s1.getName());
        s1.move();
        s1.superpower();

        System.out.println("Mob 3: " + w1.getName());
        w1.move();
        w1.superpower();

        System.out.println("Mob 4: " + s2.getName());
        s2.move();
        s2.superpower();

        System.out.println("Mob 5: " + w2.getName());
        w2.move();
        w2.superpower();
    }
}
